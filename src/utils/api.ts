 ...
 * Returns full payload so you can keep context (conversationId) and show confidence/sources.
 */
export async function sendMessage(message: string, options: SendOptions = {}): Promise<QueryResponse> {
  ...
  if (typeof options.includeMetadata === 'boolean') body.includeMetadata = options.includeMetadata;
  ...
  // Server usually returns 200 even on friendly error; still parse the JSON.
  let data: any;
  try {
    data = await res.json();
  } catch {
  ...
  if (!answer) throw new Error('No response from server');
  return response;
}
